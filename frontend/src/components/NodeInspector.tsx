...
//     try {
//       const url = buildApiUrl("/artifacts/download");
...
// --- MODIFICADO: Añadidos colores para modo oscuro (dark:...) ---
const PHASE_BADGE_MAP: Record<string, string> = {
...
export default function NodeInspector({
  ...
  const renderStructuredInput = (key: string, value: unknown) => {
    ...
    return (
      <div className="flex flex-col gap-1">
        {/* --- MODIFICADO: Textarea con soporte dark mode --- */}
        <textarea
  ...
  const renderControl = (key: string, activeValue: unknown, defaultValue: unknown) => {
    ...
    if (kind === "number") {
      const value = typeof activeValue === "number" ? activeValue : "";
      return (
        // --- MODIFICADO: Input number con dark mode ---
        <input
          type="number"
    ...
    if (kind === "boolean") {
      const value = typeof activeValue === "boolean" ? String(activeValue) : "default";
      return (
        // --- MODIFICADO: Select con dark mode ---
        <select
          className="w-full rounded border border-gray-300 bg-white px-2 py-1 text-sm text-gray-800 focus:border-indigo-400 focus:outline-none focus:ring-1 focus:ring-indigo-400 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-100"
          value={value}
          onChange={(event) => {
    ...
    if (kind === "string" || kind === "null" || kind === "unknown") {
      const value = typeof activeValue === "string" ? activeValue : "";
      return (
        // --- MODIFICADO: Input text con dark mode ---
        <input
          type="text"
          className="w-full rounded border border-gray-300 bg-white px-2 py-1 text-sm text-gray-800 focus:border-indigo-400 focus:outline-none focus:ring-1 focus:ring-indigo-400 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-100"
          value={value}
  ...
  return (
    // --- MODIFICADO: Panel principal con dark mode ---
    <aside className="relative z-20 flex h-full min-h-0 w-80 shrink-0 flex-col overflow-hidden border-l border-gray-200 bg-white text-sm text-gray-800 shadow-xl dark:border-gray-800 dark:bg-gray-900 dark:text-gray-100">
      ...
      <div className="flex-1 overflow-y-auto px-4 py-4">
        {!node ? (
          ...
          <div className="flex flex-col gap-4 pb-8">
            {isOutputNode && (
              // --- MODIFICADO: Contenedor de outputs ---
              <div className="rounded border border-indigo-200 bg-indigo-50 p-3 text-xs text-gray-700 dark:border-indigo-800 dark:bg-indigo-900/20 dark:text-gray-300">
                ...
                {outputArtifacts?.map((artifact) => {
                  ...
                  const previewState = previewStates[artifact.key];
                  return (
                    // --- MODIFICADO: Tarjeta de artefacto individual ---
                    <div
                      key={artifact.key}
                    ...
                    >
                      ...
                      <div className="mt-2 flex flex-wrap gap-2">
                        {/* --- MODIFICADO: Botones descargar/preview con soporte dark --- */}
                        <button
                          type="button"
                          onClick={() => handleDownloadArtifact(artifact)}
                          disabled={downloadStatus.loading}
